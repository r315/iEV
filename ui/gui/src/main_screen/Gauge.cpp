...
void Gauge::setBitmaps(Bitmap backgroundBmp, Bitmap centerBmp)
{
    ...
    //needleShadow.setOrigin(background.getWidth() / 2, background.getHeight() - (center.getHeight()/2)); //background.getHeight() / 2);
...
void Gauge::setValue(int newValue)
{
    ...
    if (newValue == currentValue || gaugeAnimationRunning == true){
        return;
    }
